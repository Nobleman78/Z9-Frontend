...
const UpdateInternational = () => {
    ...
    useEffect(() => {
        const fetchPackage = async () => {
            try {
                const response = await axiosSecure.get(`/international/${id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axiosSecure.put(`/international/${id}`, {
                ...formData,
    ...
    return (
        <div className='bg-white min-h-screen p-4 md:p-6 rounded-lg shadow'>
            <div className='flex flex-col md:flex-row md:items-center justify-between mb-6'>
                <h2 className='text-2xl font-bold text-sky-600 mb-4 md:mb-0'>Update International Package</h2>
                <button
                    onClick={handleBack}
